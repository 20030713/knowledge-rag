...
public class DatabaseSchemaInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        createUserTable();
        addColumnIfMissing("user", "enabled", "`enabled` TINYINT(1) NOT NULL DEFAULT 1");
        ensureAdminExists();
        ...
        createDocumentTable();
        addColumnIfMissing("document", "retry_count", "`retry_count` INT NOT NULL DEFAULT 0");
        addColumnIfMissing("document", "chunk_count", "`chunk_count` INT NOT NULL DEFAULT 0");
        addColumnIfMissing("document", "parse_duration_ms", "`parse_duration_ms` BIGINT DEFAULT NULL");
        createDocumentChunkTable();
        addColumnIfMissing("document_chunk", "embedding_model", "`embedding_model` VARCHAR(256) DEFAULT NULL");
        addColumnIfMissing("document_chunk", "embedding_json", "`embedding_json` LONGTEXT DEFAULT NULL");
        createTaskLogTable();
        createDocumentTaskQueueTable();
        createPromptTemplateTable();
        createQaRecordTable();
        addColumnIfMissing("qa_record", "session_id", "`session_id` BIGINT DEFAULT NULL");
        addColumnIfMissing("qa_record", "answer_style", "`answer_style` VARCHAR(32) NOT NULL DEFAULT 'STRICT'");
        addColumnIfMissing("qa_record", "answer_source", "`answer_source` VARCHAR(32) NOT NULL DEFAULT 'LOCAL_FALLBACK'");
        addColumnIfMissing("qa_record", "model_name", "`model_name` VARCHAR(128) DEFAULT NULL");
        addColumnIfMissing("qa_record", "latency_ms", "`latency_ms` BIGINT DEFAULT NULL");
        addColumnIfMissing("qa_record", "fallback", "`fallback` TINYINT(1) NOT NULL DEFAULT 0");
        addColumnIfMissing("qa_record", "feedback_score", "`feedback_score` INT DEFAULT NULL");
        addColumnIfMissing("qa_record", "feedback_note", "`feedback_note` VARCHAR(500) DEFAULT NULL");
    ...
    private void ensureAdminExists() {
        ...
        Long adminCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM `user` WHERE `role` = 'ADMIN'", Long.class);
        if (userCount != null && userCount > 0 && (adminCount == null || adminCount == 0)) {
            jdbcTemplate.update("""
                    UPDATE `user`
                    SET `role` = 'ADMIN', `enabled` = 1, `updated_at` = NOW()
                    ORDER BY `created_at` ASC
                    LIMIT 1
                    """);
            log.info("No admin user found, promoted the earliest user to ADMIN");
        }
    }

    private void createUserLoginLogTable() {
        jdbcTemplate.execute("""
    ...
    private void createAdminOperationLogTable() {
        jdbcTemplate.execute("""
    ...
    private void createKnowledgeBaseMemberTable() {
        jdbcTemplate.execute("""
    ...
    private void createChatSessionTable() {
        jdbcTemplate.execute("""
    ...
    private void createDocumentChunkTable() {
        jdbcTemplate.execute("""
    ...
    private void createTaskLogTable() {
        jdbcTemplate.execute("""
    ...
    private void createDocumentTaskQueueTable() {
        jdbcTemplate.execute("""
    ...
    private void createQaRecordTable() {
        jdbcTemplate.execute("""
    ...
    private void createPromptTemplateTable() {
        jdbcTemplate.execute("""
    ...
    private void createQaCitationTable() {
        jdbcTemplate.execute("""
    ...
    private void createRagEvalCaseTable() {
        jdbcTemplate.execute("""
    ...
    private void createRagEvalRunTable() {
        jdbcTemplate.execute("""
